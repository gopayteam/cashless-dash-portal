...
@Component({
  ...
  templateUrl: './update-conductor.html',
  styleUrls: ['./update-conductor.css', '../../../../../styles/global/_toast.css'],
...
export class UpdateConductorComponent implements OnInit {
    ...
    this.route.params.subscribe(params => {
      ...
      if (!id) {
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: 'Conductor ID not found in route',
          life: 4000
        });
  ...
  private getUserFromState(): User | null {
    try {
      ...
      if (navigation?.extras?.state?.['conductor']) {
  ...
  private loadUserDataFromAPI(userId: number): void {
    ...
    this.dataService
      ...
      .subscribe({
        ...
        error: (err) => {
          console.error('Failed to load conductor data from API:', err);
          this.messageService.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Failed to fetch conductor data. Please try again.',
            life: 4000
          });
          this.userLoading = false;
          this.loadingStore.stop();
    ...
    this.dataService
      ...
      .subscribe({
        next: (response) => {
          ...
          if (response.status == 0) {
            this.messageService.add({
              severity: 'success',
              summary: 'Success',
              detail: response.message || 'Conductor updated successfully',
              life: 4000
            });
          } else {
            this.messageService.add({
        ...
        error: (err) => {
          ...
          let errorMessage = 'Failed to update conductor';

          if (err.status === 409) {
            errorMessage = 'Conductor with this email or phone number already exists';
