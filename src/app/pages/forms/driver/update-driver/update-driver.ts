...
export class UpdateDriverComponent implements OnInit {
    ...
    this.route.params.subscribe(params => {
      ...
      if (!id) {
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: 'Driver ID not found in route',
          life: 4000
        });
  ...
  private initializeUserData(): void {
    ...
    if (stateUser) {
      console.log('✓ Driver data loaded from navigation state');
  ...
  private getUserFromState(): User | null {
    try {
      ...
      if (navigation?.extras?.state?.['driver']) {
        const stateUser = navigation.extras.state['driver'] as User;
        if (this.isValidUserObject(stateUser)) {
          return stateUser;
        }
      }

      if (window.history.state?.driver) {
        const historyUser = window.history.state.driver as User;
      ...
      console.error('Error retrieving driver from state:', error);
      return null;
    }
  }
  ...
  private populateFormFromUser(user: User): void {
    ...
    // console.log('Form populated with driver data:', {
    //   id: this.userId,
  ...
  private loadUserDataFromAPI(userId: number): void {
    ...
    console.log('Fetching driver data from API for ID:', userId);

    this.dataService
      .post<UserApiResponse>(API_ENDPOINTS.ALL_USERS, payload, 'get-driver-users')
      .subscribe({
        next: (response) => {
          if (response.data && response.data.length > 0) {
            ...
            if (user) {
              console.log('✓ Driver data fetched successfully from API');
        ...
        error: (err) => {
          console.error('Failed to load driver data from API:', err);
          this.messageService.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Failed to fetch driver data. Please try again.',
            life: 4000
          });
          this.userLoading = false;
          this.loadingStore.stop();
  ...
  onSubmit(): void {
    ...
    this.dataService
      .post<ApiResponse>(API_ENDPOINTS.UPDATE_USER, payload, 'update-driver-user')
      .subscribe({
        next: (response) => {
          ...
          if (response.status == 0) {
            ...
            this.messageService.add({
              severity: 'success',
              summary: 'Success',
              detail: response.message || 'Driver updated successfully',
              life: 4000
            });
          } else {
