...
export class UpdateMarshalComponent implements OnInit {
  ...
  // Constants
  readonly PROFILE = 'MARSHAL';
  readonly CHANNEL = 'PORTAL';
  readonly AGENT = 'MARSHAL';
    ...
    this.route.params.subscribe(params => {
      ...
      if (!id) {
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: 'Marshal ID not found in route',
          life: 4000
        });
        this.router.navigate(['/users/marshals']);
        return;
      }
  ...
  private initializeUserData(): void {
    ...
    if (stateUser) {
      console.log('✓ Marshal data loaded from navigation state');
  ...
  private getUserFromState(): User | null {
    try {
      ...
      if (navigation?.extras?.state?.['marshal']) {
        const stateUser = navigation.extras.state['marshal'] as User;
        if (this.isValidUserObject(stateUser)) {
          return stateUser;
        }
      }

      if (window.history.state?.marshal) {
        const historyUser = window.history.state.marshal as User;
      ...
      console.error('Error retrieving marshal from state:', error);
      return null;
    }
  }
  ...
  private populateFormFromUser(user: User): void {
    ...
    // console.log('Form populated with marshal data:', {
    //   id: this.userId,
  ...
  private loadUserDataFromAPI(userId: number): void {
    ...
    console.log('Fetching marshal data from API for ID:', userId);

    this.dataService
      .post<UserApiResponse>(API_ENDPOINTS.ALL_USERS, payload, 'get-marshal-users')
      .subscribe({
        next: (response) => {
          if (response.data && response.data.length > 0) {
            ...
            if (user) {
              console.log('✓ Marshal data fetched successfully from API');
        ...
        error: (err) => {
          console.error('Failed to load marshal data from API:', err);
          this.messageService.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Failed to fetch marshal data. Please try again.',
            life: 4000
          });
          this.userLoading = false;
          this.loadingStore.stop();
        ...
        next: (response) => {
          ...
          if (response.status == 0) {
            this.messageService.add({
              severity: 'success',
              summary: 'Success',
              detail: response.message || 'Marshal updated successfully',
              life: 4000
            });
          } else {
            this.messageService.add({
