...
@Component({
  ...
  styleUrls: ['./register-passenger.css', '../../../../../styles/global/_toast.css'],
...
export class RegisterPassengerComponent implements OnInit {
  ...
  onSubmit(): void {
    ...
    this.dataService
      .post<ApiResponse>(API_ENDPOINTS.REGISTER_USER, payload, 'register-passenger-user')
      .subscribe({
        next: (response) => {
          console.log('Passenger created successfully', response);
          ...
          if (response.status == 0) {
            this.messageService.add({
              severity: 'success',
              summary: 'Success',
              detail: response.message || 'Passenger created successfully',
              life: 4000
            });
          } else {
            this.messageService.add({
