...
@Component({
  selector: 'app-update-dashmaster',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './update-dashmaster.html',
  styleUrls: ['./update-dashmaster.css', '../../../../../styles/global/_toast.css'],
  providers: [MessageService]
})
export class UpdateDashmasterComponent implements OnInit {
  entityId: string | null = null;
  userId: number | null = null;
  userData: User | null = null;

  // Constants
  readonly PROFILE = 'DASHMASTER';
  readonly CHANNEL = 'PORTAL';
  readonly AGENT = 'DASHMASTER';
    ...
    this.route.params.subscribe(params => {
      ...
      if (!id) {
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: 'Dashmaster ID not found in route',
          life: 4000
        });
        this.router.navigate(['/users/dashmasters']);
        return;
      }
  ...
  private initializeUserData(): void {
    ...
    if (stateUser) {
      console.log('✓ Dashmaster data loaded from navigation state');
  ...
  private getUserFromState(): User | null {
    try {
      ...
      if (navigation?.extras?.state?.['dashmaster']) {
        const stateUser = navigation.extras.state['dashmaster'] as User;
        if (this.isValidUserObject(stateUser)) {
          return stateUser;
        }
      }

      if (window.history.state?.dashmaster) {
        const historyUser = window.history.state.dashmaster as User;
      ...
      console.error('Error retrieving dashmaster from state:', error);
      return null;
    }
  }
  ...
  private populateFormFromUser(user: User): void {
    ...
    // console.log('Form populated with dashmaster data:', {
    //   id: this.userId,
  ...
  private loadUserDataFromAPI(userId: number): void {
    ...
    console.log('Fetching dashmaster data from API for ID:', userId);

    this.dataService
      .post<UserApiResponse>(API_ENDPOINTS.ALL_USERS, payload, 'get-dashmaster-users')
      .subscribe({
        next: (response) => {
          if (response.data && response.data.length > 0) {
            ...
            if (user) {
              console.log('✓ Dashmaster data fetched successfully from API');
        ...
        error: (err) => {
          console.error('Failed to load dashmaster data from API:', err);
          this.messageService.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Failed to fetch dashmaster data. Please try again.',
            life: 4000
          });
          this.userLoading = false;
          this.loadingStore.stop();
        ...
        next: (response) => {
          ...
          if (response.status == 0) {
            ...
            this.messageService.add({
              severity: 'success',
              summary: 'Success',
              detail: response.message || 'Dashmaster updated successfully',
              life: 4000
            });
          } else {
