...
@Component({
  ...
  templateUrl: './update-inspector.html',
  styleUrls: ['./update-inspector.css', '../../../../../styles/global/_toast.css'],
...
export class UpdateInspectorComponent implements OnInit {
  ...
  // Constants
  readonly PROFILE = 'INSPECTOR';
  readonly CHANNEL = 'PORTAL';
  readonly AGENT = 'INSPECTOR';
    ...
    this.route.params.subscribe(params => {
      ...
      if (!id) {
        this.messageService.add({
          severity: 'error',
          summary: 'Error',
          detail: 'Inspector ID not found in route',
          life: 4000
        });
        this.router.navigate(['/users/inspectors']);
        return;
      }
  ...
  private initializeUserData(): void {
    ...
    if (stateUser) {
      console.log('✓ Inspector data loaded from navigation state');
  ...
  private getUserFromState(): User | null {
    try {
      ...
      if (navigation?.extras?.state?.['inspector']) {
        const stateUser = navigation.extras.state['inspector'] as User;
        if (this.isValidUserObject(stateUser)) {
          return stateUser;
        }
      }

      if (window.history.state?.inspector) {
        const historyUser = window.history.state.inspector as User;
      ...
      console.error('Error retrieving inspector from state:', error);
      return null;
    }
  }
  ...
  private populateFormFromUser(user: User): void {
    ...
    // console.log('Form populated with Inspector data:', {
    //   id: this.userId,
  ...
  private loadUserDataFromAPI(userId: number): void {
    ...
    console.log('Fetching inspector data from API for ID:', userId);

    this.dataService
      .post<UserApiResponse>(API_ENDPOINTS.ALL_USERS, payload, 'get-inspector-users')
      .subscribe({
        next: (response) => {
          if (response.data && response.data.length > 0) {
            ...
            if (user) {
              console.log('✓ Inspector data fetched successfully from API');
        ...
        error: (err) => {
          console.error('Failed to load Inspector data from API:', err);
          this.messageService.add({
            severity: 'error',
            summary: 'Error',
            detail: 'Failed to fetch Inspector data. Please try again.',
            life: 4000
          });
          this.userLoading = false;
          this.loadingStore.stop();
  ...
  onSubmit(): void {
    ...
    this.dataService
      .post<ApiResponse>(API_ENDPOINTS.UPDATE_USER, payload, 'update-inspector-user')
      .subscribe({
        next: (response) => {
          ...
          if (response.status == 0) {
            console.log('inspector updated successfully', response);
            this.messageService.add({
              severity: 'success',
              summary: 'Success',
              detail: response.message || 'Inspector updated successfully',
              life: 4000
            });
          } else {
