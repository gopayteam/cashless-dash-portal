...
@Component({
  selector: 'app-all-dashmaster-users',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class DashmastersComponent implements OnInit {
  ...
  fetchUsers(bypassCache: boolean, $event?: any): void {
    ...
    this.dataService
      .post<UserApiResponse>(API_ENDPOINTS.ALL_USERS, payload, 'dashmaster-users', bypassCache)
      .subscribe({
        next: (response) => {
        ...
        error: (err) => {
          console.error('Failed to load dashmaster users', err);
          this.loadingStore.stop();
        },
      });
  }
  ...
  navigateToCreateDashmaster(): void {
    this.router.navigate(['forms/register-dashmaster'])
  }

  navigateToUpdateDashmaster(dashmaster: User): void {
    event?.stopPropagation();

    if (!dashmaster?.id) {
      console.error('Dashmaster ID missing', dashmaster);
      return;
    }

    // console.log('Navigating to update dashmaster:', dashmaster.id);
    // console.log('Dashmaster data being passed:', dashmaster);

    // Pass the complete user object through router state
    // This ensures the data is immediately available in the update component
    this.router.navigate(['/forms/update-dashmaster', dashmaster.id], {
      state: {
        dashmaster: dashmaster,
