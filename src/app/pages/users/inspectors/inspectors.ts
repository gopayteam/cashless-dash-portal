...
export class InspectorsComponent implements OnInit {
  ...
  fetchUsers(bypassCache: boolean, $event?: any): void {
    ...
    this.dataService
      .post<UserApiResponse>(API_ENDPOINTS.ALL_USERS, payload, 'inspector-users', bypassCache)
      .subscribe({
        next: (response) => {
  ...
  navigateToCreateInspector(): void {
    this.router.navigate(['forms/register-inspector'])
  }

  navigateToUpdateInspector(inspector: User): void {
    event?.stopPropagation();

    if (!inspector?.id) {
      console.error('Inspector ID missing', inspector);
      return;
    }

    // console.log('Navigating to update inspector:', inspector.id);
    // console.log('Inspector data being passed:', inspector);

    // Pass the complete user object through router state
    // This ensures the data is immediately available in the update component
    this.router.navigate(['/forms/update-inspector', inspector.id], {
      state: {
        inspector: inspector,
