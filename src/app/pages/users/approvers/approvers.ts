...
@Component({
  selector: 'app-all-approver-users',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
...
export class ApproversComponent implements OnInit {
  ...
  fetchUsers(bypassCache: boolean, $event?: any): void {
    ...
    this.dataService
      .post<UserApiResponse>(API_ENDPOINTS.ALL_USERS, payload, 'approver-users', bypassCache)
      .subscribe({
        next: (response) => {
  ...
  navigateToCreateApprover(): void {
    this.router.navigate(['forms/register-approver'])
  }

  navigateToUpdateApprover(approver: User): void {
    event?.stopPropagation();

    if (!approver?.id) {
      console.error('Approver ID missing', approver);
      return;
    }

    // console.log('Navigating to update approver:', approver.id);
    // console.log('Approver data being passed:', approver);

    // Pass the complete user object through router state
    // This ensures the data is immediately available in the update component
    this.router.navigate(['/forms/update-approver', approver.id], {
      state: {
        approver: approver,
