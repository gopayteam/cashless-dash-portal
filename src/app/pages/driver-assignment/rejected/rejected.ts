...
@Component({
  ...
  styleUrls: ['./rejected.css', '../../../../styles/global/_toast.css'],
...
export class AllRejectedDriverAssignmentsComponent implements OnInit {
  ...
  exportToExcel(): void {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Rejected Drivers');

      const filename = `rejected_driver_assignments_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  exportToCSV(): void {
    ...
    try {
      ...
      const filename = `rejected_driver_assignments_${new Date().toISOString().split('T')[0]}.csv`;
